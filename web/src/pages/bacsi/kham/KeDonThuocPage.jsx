  ...
  const loadInitialData = useCallback(async () => {
    ...
    try {
      ...
      setSelectedRecordId((current) =>
        current && recordItems.some((item) => item.recordId === current)
          ? current
          : "",
      );
    } catch (error) {
        ...
        <div>
          ...
          {selectedRecord && (
            <div className="mt-3 rounded-lg bg-slate-50 p-3 text-xs text-slate-600">
              ...
              <p>
                Mã hồ sơ: {selectedRecord.displayRecordId || selectedRecord.recordId}
                {selectedRecord.cccd ? ` • CCCD: ${selectedRecord.cccd}` : ""}
              </p>
            </div>
          )}
        </div>

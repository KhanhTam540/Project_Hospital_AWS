...
async function requireDoctorOwnership(event, doctorId) {
  ...
  const profile = await ensureApplicationProfile(event);
  if (profile.appUser.doctorId !== doctorId) {
    throw new ApiError(403, 'FORBIDDEN', 'Bác sĩ chỉ được truy cập lịch của chính mình');
  }
}
...
async function handleDoctorByAccount(event) {
  ...
  const item = await repo.findOneByField('DOCTOR', 'accountUserId', accountId);
  if (!item) throw new ApiError(404, 'DOCTOR_NOT_FOUND', 'Không tìm thấy bác sĩ theo tài khoản');
  return success(domain.mapDoctor(item));
}
...
async function handleCreateSchedule(event) {
  const actor = requireGroups(event, ['ADMIN', 'NHANSU']);
  const payload = domain.normalizeSchedulePayload(parseJsonBody(event));
...
async function handleListAppointments(event) {
  ...
  if (
    ...
    items = await repo.listAppointmentsByDoctor(profile.appUser.doctorId);
  } else {
...
async function handleCreateAppointment(event) {
  ...
  let payload = domain.normalizeAppointmentPayload(parseJsonBody(event));

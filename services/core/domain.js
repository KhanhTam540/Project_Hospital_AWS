...
function normalizeCitizenId(value, { required = false } = {}) {
  ...
  if (!normalized) {
    if (required) throw new ValidationError('CCCD là bắt buộc');
    return null;
  }
  if (!/^\d{12}$/.test(normalized)) {
    throw new ValidationError('CCCD phải gồm đúng 12 chữ số');
  }
  return normalized;
}
...
function normalizeAccountPayload(body = {}, { isCreate = true } = {}) {
  ...
  return {
    ...
    citizenId: normalizeCitizenId(body.cccd || body.citizenId),
  };
}

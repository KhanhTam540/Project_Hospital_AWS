...
function normalizeCitizenId(value, { required = false } = {}) {
  const normalized = String(value ?? '').trim();
  if (!normalized) {
    if (required) {
      throw new ApiError(400, 'VALIDATION_ERROR', 'CCCD là bắt buộc');
    }
    return null;
  }
...
function mapMedicalRecord(item = {}, patientMap = new Map()) {
  ...
  const displayRecordId = item.recordCode || patient?.citizenId || recordId;
  ...
  return {
    ...
    maBN: patientId,
    patientId,
    cccd: patient?.citizenId || item.citizenId || '',
...
function mapMedicalRecordCover(patient = {}, record = null) {
  ...
  const citizenId = patient.citizenId || record?.citizenId || null;
  const createdAt =
    record?.createdAt ||
    patient.createdAt ||
    null;
  ...
  // Với bệnh nhân chưa có hồ sơ vật lý, dùng CCCD làm mã hồ sơ dự kiến.
  // patientId nội bộ vẫn được giữ để không phá vỡ các quan hệ hiện có.
  const internalRecordId =
    record?.recordId || citizenId || `HSBA-${patientId}`;
  const displayRecordId = citizenId || record?.recordCode || internalRecordId;
  const mappedPatient = mapPatient(patient);

  return {
    maHSBA: internalRecordId,
    medicalRecordId: internalRecordId,
    maHSBAHienThi: displayRecordId,
    displayRecordId,
    recordId: internalRecordId,
    recordCode: displayRecordId,
    cccd: citizenId || '',
    citizenId: citizenId || '',
    maBN: patientId,
    patientId,
    patientName: mappedPatient.hoTen || '',
    ngayLap: createdAt,
    createdAt,
...
async function resolveCurrentPatientId(event) {
  const current = getCurrentUser(event);
  if (!current.groups.includes('BENHNHAN')) return null;
  ...
  const directPatient = patients.find((patient) =>
    [
      patient.cognitoSub,
      patient.cognitoUsername,
      patient.accountUserId,
      patient.email,
    ].some((value) =>
      currentValues.some((currentValue) => sameIdentifier(value, currentValue)),
    ),
  );
  if (directPatient?.patientId) return directPatient.patientId;
...
async function requirePatientSelfOrAdmin(event, patientId) {
  const current = requireGroups(event, ['ADMIN', 'BENHNHAN']);
  if (current.groups.includes('ADMIN')) return current;

  const resolvedPatientId = await resolveCurrentPatientId(event);
  if (!resolvedPatientId || !sameIdentifier(resolvedPatientId, patientId)) {
    throw new ApiError(
      403,
      'FORBIDDEN',
      'Bệnh nhân chỉ được cập nhật thông tin của chính mình',
    );
  }
  return current;
}

async function assertUniqueCitizenId(citizenId, excludedPatientId = null) {
  if (!citizenId) return;
  const patients = await scanEntityTypes(['PATIENT']);
  const duplicate = patients.find(
    (item) =>
      sameIdentifier(item.citizenId, citizenId) &&
      !sameIdentifier(item.patientId, excludedPatientId),
  );
  if (duplicate) {
    throw new ApiError(
      409,
      'CITIZEN_ID_ALREADY_EXISTS',
      'Số CCCD đã được sử dụng cho bệnh nhân khác',
    );
  }
}

async function ensureMedicalRecordForCitizenId(patientId, citizenId) {
  const normalizedCitizenId = normalizeCitizenId(citizenId, {
    required: true,
  });
  const records = (await scanMedicalRecords()).filter((item) =>
    sameIdentifier(item.patientId, patientId),
  );
  ...
  if (records.length > 0) {
    ...
    for (const record of records) {
      const updated = {
        ...record,
        recordCode: normalizedCitizenId,
    ...
    return pickCanonicalMedicalRecord(synchronized);
  }

  const now = nowIso();
  const item = {
    pk: `PATIENT#${patientId}`,
    sk: `MEDICAL_RECORD#${normalizedCitizenId}`,
    entityType: 'MEDICAL_RECORD',
    recordId: normalizedCitizenId,
    recordCode: normalizedCitizenId,
    citizenId: normalizedCitizenId,
    patientId,
    doctorId: null,
...
async function getPatient(event) {
  ...
  if (current.groups.includes('BENHNHAN')) {
    const resolvedPatientId = await resolveCurrentPatientId(event);
    if (!resolvedPatientId || !sameIdentifier(resolvedPatientId, patientId)) {
      throw new ApiError(403, 'FORBIDDEN', 'Bạn chỉ được xem hồ sơ của chính mình');
    }
  }
...
async function createPatient(event) {
  ...
  const patientId = body.maBN
    ...
    : generatedId('BN');
  const citizenId = normalizeCitizenId(body.cccd || body.citizenId);
  await assertUniqueCitizenId(citizenId);
  const now = nowIso();
  const item = {
    pk: `PATIENT#${patientId}`,
    sk: 'PROFILE',
  ...
  await putItem(item, { createOnly: true });
  if (citizenId) {
    await ensureMedicalRecordForCitizenId(patientId, citizenId);
  }
...
async function updatePatient(event) {
  const patientId = normalizeCode(routeParameter(event, 'patientId'), 'maBN');
  const actor = await requirePatientSelfOrAdmin(event, patientId);
  ...
  const body = parseJsonBody(event);
  const citizenId = body.cccd !== undefined || body.citizenId !== undefined
    ? normalizeCitizenId(body.cccd || body.citizenId, { required: true })
    : existing.citizenId || null;
  await assertUniqueCitizenId(citizenId, patientId);

  const isAdmin = actor.groups.includes('ADMIN');
  if (
    !isAdmin &&
    existing.citizenId &&
    !sameIdentifier(existing.citizenId, citizenId)
  ) {
    throw new ApiError(
      409,
      'CITIZEN_ID_IMMUTABLE',
      'CCCD đã xác nhận không thể tự thay đổi. Vui lòng liên hệ quản trị viên.',
    );
  }
  ...
  const updated = {
    ...
    citizenId,
    status: isAdmin
      ? (Number(body.trangThai ?? (existing.status === 'INACTIVE' ? 0 : 1)) === 0
          ? 'INACTIVE'
          : 'ACTIVE')
...
async function listMedicalRecords(event) {
  ...
  const requestedCitizenIdRaw =
    event.queryStringParameters?.cccd ||
    event.queryStringParameters?.citizenId ||
    null;
  const requestedCitizenId = requestedCitizenIdRaw
    ? normalizeCitizenId(requestedCitizenIdRaw, { required: true })
    : null;
  ...
  const filteredPatients = requestedCitizenId
    ? patients.filter((patient) =>
        sameIdentifier(patient.citizenId, requestedCitizenId),
      )
    : patients;
  ...
  // Bác sĩ và nhân sự tra cứu một bệnh nhân duy nhất bằng CCCD.
  // Bệnh nhân đã có CCCD vẫn xuất hiện ngay cả khi hồ sơ vật lý chưa tồn tại;
  // hồ sơ sẽ được tạo tự động khi chọn/lập phiếu khám lần đầu.
  const covers = filteredPatients
    .filter((patient) => /^\d{12}$/.test(String(patient.citizenId || '')))
    .map((patient) =>
      mapMedicalRecordCover(
        patient,
        pickCanonicalMedicalRecord(
    ...
    .sort((left, right) =>
      String(left.cccd || '').localeCompare(String(right.cccd || '')),
    );
...
async function createMedicalRecord(event) {
  ...
  const patient = await requireEntity(
    'PATIENT',
    'patientId',
    patientId,
    'PATIENT_NOT_FOUND',
  ...
  const citizenId = normalizeCitizenId(patient.citizenId, { required: true });
  ...
  const requestedRecordId = body.maHSBA
    ? normalizeCode(body.maHSBA, 'maHSBA')
    : citizenId;
  if (!sameIdentifier(requestedRecordId, citizenId)) {
    throw new ApiError(
      400,
      'MEDICAL_RECORD_ID_MUST_MATCH_CCCD',
      'Mã hồ sơ bệnh án phải trùng với số CCCD của bệnh nhân',
    );
  }
  const recordId = citizenId;
  const now = nowIso();
  const item = {
    pk: `PATIENT#${patientId}`,

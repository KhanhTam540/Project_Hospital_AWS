...
async function createExamination(event) {
  ...
  if (medicalRecordId) {
    await ensureDirectEntity('RECORD', medicalRecordId, patientId);
  }
...
async function listExaminations(event) {
  ...
  if (requestedRecordId) {
    await ensureDirectEntity('RECORD', requestedRecordId, patientId);
...
async function createPrescription(event) {
  ...
  const record = await ensureDirectEntity('RECORD', recordId, patientId);
...
async function getPatientMedicalTimeline(event) {
  ...
  const records = uniqueBy(
    allItems.filter(
      (item) => item.entityType === 'MEDICAL_RECORD' && item.patientId === patientId,
    ),
    (item) => item.recordId,
  ).sort((left, right) =>
    String(left.createdAt || left.encounterAt || '').localeCompare(
  ...
  for (const appointment of appointments) {
    ...
    events.push({
      ...
      thoiGian: occurredAt,
      occurredAt,
      maHSBA: records[0]?.recordId || null,
  ...
  const recordId = firstRecord?.recordId || `HSBA-${patientId}`;
  const diagnoses = [

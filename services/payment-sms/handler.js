...
async function createInvoiceFromBody(event, body, cartItems = []) {
  ...
  const medicalRecordId = body.medicalRecordId || body.maHSBA
...
async function manualPayment(event) {
  const user = requireGroups(event, ['ADMIN', 'NHANSU']);
  const body = parseJsonBody(event);
  ...
  if (!['CASH', 'BANK_TRANSFER'].includes(provider)) {
    throw new ApiError(400, 'ONLINE_PROVIDER_REQUIRED', 'Use /api/payment/create-url for VNPAY or MOMO');
  }
  const result = await recordPayment({
    invoice,
    provider,
    providerTransactionId: body.transactionId || id('TX'),
    requestIdValue: body.requestId || null,
    status: 'SUCCESS',
    amount: body.amount || body.soTien || invoice.totalAmount,
    raw: { confirmedBy: user.sub },
  });
  return success({ payment: mapPayment(result.payment), invoice: mapInvoice(result.invoice), duplicate: result.duplicate }, 201);
}

...
class QuanLyDonViTinhScreen extends StatefulWidget {
  const QuanLyDonViTinhScreen({super.key});

  @override
  State<QuanLyDonViTinhScreen> createState() => _QuanLyDonViTinhScreenState();
}
...
class _QuanLyDonViTinhScreenState extends State<QuanLyDonViTinhScreen> {
  ...
  Future<void> _handleDelete(String maDVT) async {
    final confirm = await _showConfirmDialog(
      'XÃ¡c nháº­n xoÃ¡',
      'Báº¡n cÃ³ cháº¯c muá»‘n xoÃ¡ $maDVT?',
    );
  ...
  Future<void> _showAddEditDialog({DonViTinhModel? dvt}) async {
    ...
    final tenController = TextEditingController(text: dvt?.tenDVT);
    final moTaController = TextEditingController(text: dvt?.moTa);

    showDialog(
      context: context,
      builder: (ctx) {
        return AlertDialog(
          title: Text(isEdit ? 'Sá»­a ÄÆ¡n vá»‹' : 'ThÃªm ÄÆ¡n vá»‹'),
          content: Form(
            key: formKey,
            child: Column(
              ...
              children: [
                TextFormField(
                  ...
                  decoration: InputDecoration(
                    labelText: 'TÃªn Ä‘Æ¡n vá»‹ (viÃªn, vá»‰...)',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Quáº£n lÃ½ ÄÆ¡n vá»‹ tÃ­nh'),

...
class HospitalStack extends Stack {
  constructor(scope, id, props = {}) {
    ...
    const projectName = 'hospital-p2tb';
    ...
    const dataKey = new kms.Key(this, 'HospitalDataKey', {
      ...
      description: 'Encrypts Hospital_P2TB development data at rest.',
    ...
    const userPool = new cognito.UserPool(this, 'HospitalUserPool', {
      ...
      userVerification: {
        emailSubject: 'Hospital_P2TB - Mã xác nhận đăng ký',
        emailBody: [
          'Xin chào,',
          '',
          'Mã xác nhận đăng ký tài khoản Hospital_P2TB của bạn là {####}.',
          '',
    ...
    const groups = [
      { id: 'AdminGroup', name: 'ADMIN', description: 'Quản trị viên hệ thống.', precedence: 0 },
      { id: 'DoctorGroup', name: 'BACSI', description: 'Bác sĩ.', precedence: 10 },
      { id: 'StaffGroup', name: 'NHANSU', description: 'Nhân sự y tế và tiếp đón.', precedence: 20 },
      { id: 'PatientGroup', name: 'BENHNHAN', description: 'Bệnh nhân.', precedence: 30 },
    ];
    ...
    const createFunction = ({
      ...
      new NodejsFunction(this, functionId, {
        ...
        entry: path.join(__dirname, '..', 'services', directory, 'handler.js'),
        handler: 'handler',
        memorySize,
        timeout: Duration.seconds(timeoutSeconds),
        tracing: lambda.Tracing.ACTIVE,
        environment: { LOG_LEVEL: 'INFO', ...environment },
    ...
    const coreIntegration = new HttpLambdaIntegration('CoreIntegration', coreFunction);
    const medicalIntegration = new HttpLambdaIntegration('MedicalIntegration', medicalFunction);
    ...
    addProtectedRoute('/api/admin/ping', [apigwv2.HttpMethod.GET], coreIntegration);
    addProtectedRoute('/api/patients', [apigwv2.HttpMethod.POST], medicalIntegration);
    addProtectedRoute('/api/patients/{patientId}', [apigwv2.HttpMethod.GET], medicalIntegration);
    addProtectedRoute(
      '/api/patients/{patientId}/records',
      [apigwv2.HttpMethod.GET, apigwv2.HttpMethod.POST],
      medicalIntegration,
    );
    addProtectedRoute('/api/medical/upload-url', [apigwv2.HttpMethod.POST], medicalIntegration);
    addProtectedRoute('/api/medical/complete-upload', [apigwv2.HttpMethod.POST], medicalIntegration);
    addProtectedRoute('/api/medical/download-url', [apigwv2.HttpMethod.GET], medicalIntegration);
...
function handler(event) {
  ...
  return request;
      ...
      distribution = new cloudfront.Distribution(this, 'HospitalDistribution', {
        comment: 'Hospital_P2TB frontend and API distribution.',
    ...
    const outputs = {
      Region: this.region,
      CloudFrontEnabled: String(enableCloudFront),

...
class HospitalStack extends Stack {
  constructor(scope, id, props = {}) {
          ...
          generateSecretString: {
            secretStringTemplate:
              JSON.stringify({
                ...
                AMB_ETHEREUM_RPC_URL:
                  'SET_IN_AWS_CONSOLE',

                AMB_ETHEREUM_PRIVATE_KEY:
                  'SET_IN_AWS_CONSOLE',

                AMB_ETHEREUM_CHAIN_ID:
                  'SET_IN_AWS_CONSOLE',
              }),
    ...
    const userPool =
      new cognito.UserPool(
        ...
        'HospitalUserPool',
        {
          ...
          customAttributes: {
            cccd: new cognito.StringAttribute({
              minLen: 12,
              maxLen: 12,
              mutable: false,
            }),
    ...
    /* =========================================================
     * WEEK 2: MEDICAL-RECORD BLOCKCHAIN AUDIT
     * =======================================================*/

    const medicalAuditFunction =
      createFunction({
        id: 'MedicalAuditFunction',
        directory: 'medical-audit',
    ...
    medicalAuditFunction.addEventSource(
    ...
    /* =========================================================
     * WEEK 3: PAYMENT AND SMS OTP
     * =======================================================*/

    const paymentSmsFunction =
      createFunction({
        id: 'PaymentSmsFunction',
        directory: 'payment-sms',
        ...
        environment: {
          ...
          REQUIRE_PAYMENT_OTP:
            process.env.REQUIRE_PAYMENT_OTP ||
            'false',
          ...
          VNPAY_RETURN_URL:
            ...
            (existingCloudFrontDomain
              ? `https://${existingCloudFrontDomain}/api/payment/vnpay-return`
              : ''),

          PAYMENT_RESULT_URL:
            process.env.PAYMENT_RESULT_URL ||
            (existingCloudFrontDomain
              ? `https://${existingCloudFrontDomain}/payment-result`
    ...
    paymentSmsFunction.addToRolePolicy(
      new iam.PolicyStatement({
        actions: ['sns:Publish'],
        resources: ['*'],
      }),
    ...
    if (enableApiRoutes) {
      ...
      const medicalAuditIntegration =
        new HttpLambdaIntegration(
          'MedicalAuditIntegration',
      ...
      const paymentSmsIntegration =
        new HttpLambdaIntegration(
          'PaymentSmsIntegration',
      ...
      addProtectedRoute(
        '/api/medical-records/{recordId}/verify',
        [apigwv2.HttpMethod.POST],
        medicalAuditIntegration,
      );
      ...
      /* -------------------------
       * PAYMENT AND SMS ROUTES
       * -----------------------*/

      const billingCollectionRoutes = [
        '/api/hoadon',
        '/api/hoadon/giohang',
      ...
      for (const routePath of billingCollectionRoutes) {
        addProtectedRoute(
          routePath,
          [
            ...(routePath === '/api/hoadon'
              ? [apigwv2.HttpMethod.GET]
              : []),
            apigwv2.HttpMethod.POST,
          ],
      ...
      addProtectedRoute(
        '/api/hoadon/giohang/item/{itemId}',
        [apigwv2.HttpMethod.DELETE],
        paymentSmsIntegration,
      );

      addProtectedRoute(
        '/api/hoadon/myhoadon/{patientId}',
        [apigwv2.HttpMethod.GET],
        paymentSmsIntegration,
      );
  ...
  return request;
    ...
    const outputs = {
      ...
      MedicalAuditFunctionName:
        medicalAuditFunction.functionName,

      PaymentSmsFunctionName:
        paymentSmsFunction.functionName,

      VnpayReturnUrl:
        process.env.VNPAY_RETURN_URL ||
        (existingCloudFrontDomain
          ? `https://${existingCloudFrontDomain}/api/payment/vnpay-return`
          : 'SET_VNPAY_RETURN_URL'),

      VnpayIpnUrl:
        (existingCloudFrontDomain
          ? `https://${existingCloudFrontDomain}/api/payment/vnpay-ipn`
          : `${httpApi.apiEndpoint}/api/payment/vnpay-ipn`),
